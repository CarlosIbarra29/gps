...
class Application_Model_GpsGasolinaModel extends Zend_Db_Table_Abstract{
    ...
    public function getstepuno(){ 
        try{
            $db = Zend_Db_Table::getDefaultAdapter();
            $qry = $db->query("SELECT g.id, g.id_sitios, g.name_sitio, g.id_proyecto, g.id_responsable, 
                        g.name_responsable, g.fecha_requerida, g.id_vehiculo, g.estacion,  g.forma_pago, 
                        g.step_gasolina, g.consumo
				        FROM add_gasolina g 
    ...
    public function getprocesogasolina($estado){ 
        try{
            ...
            $qry = $db->query("SELECT g.id, g.id_sitios, g.name_sitio, g.id_proyecto, g.id_responsable, 
                        g.name_responsable, g.fecha_requerida, g.id_vehiculo, g.estacion,  g.forma_pago, 
                        g.step_gasolina, g.consumo, g.status_gasolina, g.tarjeta, g.placas, v.marca, v.modelo, 
                        v.submarca, g.importe, g.tarjeta
                        FROM add_gasolina g 
    ...
    public function getprocesogasolinasitio($estado,$nombre){ 
        try{
            ...
            $qry = $db->query("SELECT g.id, g.id_sitios, g.name_sitio, g.id_proyecto, g.id_responsable, 
                        g.name_responsable, g.fecha_requerida, g.id_vehiculo, g.estacion,  g.forma_pago, 
                        g.step_gasolina, g.importe, g.consumo, g.status_gasolina 
                        FROM add_gasolina g 
                        WHERE g.step_gasolina = 1 AND name_sitio like '%{$nombre}%' 
                        AND g.status_gasolina = ? ", array($estado));
            $row = $qry->fetchAll();
            return $row;
            $db->closeConnection();
        }catch (Exception $e){
    ...
    public function getprocesogasolinaresponsable($estado,$nombre){ 
        try{
            ...
            $qry = $db->query("SELECT g.id, g.id_sitios, g.name_sitio, g.id_proyecto, g.id_responsable, 
                        g.name_responsable, g.fecha_requerida, g.id_vehiculo, g.estacion,  g.forma_pago, 
                        g.step_gasolina, g.importe, g.consumo, g.status_gasolina FROM add_gasolina g 
                        WHERE g.step_gasolina = 1 
                        AND name_responsable like '%{$nombre}%' 
                        AND g.status_gasolina = ? ", array($estado));
            $row = $qry->fetchAll();
            return $row;
            $db->closeConnection();
    ...
    public function getprocesogasolinaplacas($estado,$nombre){ 
        try{
            $db = Zend_Db_Table::getDefaultAdapter();
            $qry = $db->query("SELECT g.id, g.id_sitios, g.name_sitio, g.id_proyecto, g.id_responsable, 
                        g.name_responsable, g.fecha_requerida, g.id_vehiculo, g.placas, g.estacion,  g.forma_pago, 
                        g.step_gasolina, g.importe,g.consumo, g.status_gasolina 
                        FROM add_gasolina g 
    ...
    public function getprocesogasolinatarjeta($estado,$nombre){ 
        try{
            ...
            $qry = $db->query("SELECT g.id, g.id_sitios, g.name_sitio, g.id_proyecto, g.id_responsable, 
                        g.name_responsable, g.fecha_requerida, g.id_vehiculo, g.placas, g.estacion,  g.forma_pago, 
                        g.step_gasolina, g.importe,g.consumo, g.status_gasolina, g.id_tarjeta 
                        FROM add_gasolina g 
                        WHERE g.step_gasolina = 1 AND g.id_tarjeta = ?
                        AND g.status_gasolina = ? ", array($nombre,$estado));
            $row = $qry->fetchAll();
            return $row;
            $db->closeConnection();
        }catch (Exception $e){
    ...
    public function getprocesopaginator($offset,$no_of_records_per_page,$estado){
        try{
            $db = Zend_Db_Table::getDefaultAdapter();
            $qry = $db->query("SELECT g.id, g.id_sitios, g.name_sitio, g.id_proyecto, g.id_responsable, 
                        g.name_responsable, g.fecha_requerida, g.id_vehiculo, g.estacion, g.forma_pago, g.importe, 
    ...
    public function getprocesopaginatorsitio($offset,$no_of_records_per_page,$estado,$nombre){
        try{
            ...
            $qry = $db->query("SELECT g.id, g.id_sitios, g.name_sitio, g.id_proyecto, g.id_responsable, 
                        g.name_responsable, g.fecha_requerida, g.id_vehiculo, g.estacion, g.forma_pago, g.importe, 
                        g.step_gasolina, g.consumo, g.status_gasolina, g.id_vehiculo, v.placas
                        FROM add_gasolina g 
                        LEFT JOIN vehiculos v on g.id_vehiculo = v.id_vehiculos 
                        WHERE g.step_gasolina = 1 AND name_sitio like '%{$nombre}%'
    ...
    public function getprocesopaginatorplacas($offset,$no_of_records_per_page,$estado,$nombre){
        try{
            ...
            $qry = $db->query("SELECT g.id, g.id_sitios, g.name_sitio, g.id_proyecto, g.id_responsable, 
                        g.name_responsable, g.fecha_requerida, g.id_vehiculo,g.placas, g.estacion, g.forma_pago, 
                        g.importe, g.step_gasolina, g.consumo, g.status_gasolina, g.id_vehiculo, v.placas
                        FROM add_gasolina g 
                        LEFT JOIN vehiculos v on g.id_vehiculo = v.id_vehiculos 
                        WHERE g.step_gasolina = 1 AND g.placas like '%{$nombre}%'
                        AND g.status_gasolina = ? LIMIT $offset,$no_of_records_per_page",array($estado));
    ...
    public function getprocesopaginatortarjeta($offset,$no_of_records_per_page,$estado,$nombre){
        try{
            ...
            $qry = $db->query("SELECT g.id, g.id_sitios, g.name_sitio, g.id_proyecto, g.id_responsable, 
                        g.name_responsable, g.fecha_requerida, g.id_vehiculo,g.placas, g.estacion, g.forma_pago, 
                        g.importe, g.step_gasolina, g.consumo, g.status_gasolina, g.id_tarjeta, v.placas
                        FROM add_gasolina g 
                        LEFT JOIN vehiculos v on g.id_vehiculo = v.id_vehiculos 
                        WHERE g.step_gasolina = 1 AND g.id_tarjeta = ? 
    ...
    public function getefecticardpaginator($offset,$no_of_records_per_page){
        try{
            $db = Zend_Db_Table::getDefaultAdapter();
            $qry = $db->query("SELECT t.id, t.no_tarjeta,t.id_responsable, t.status_tarjeta, t.fecha_asignacion, 
                        t.vigencia,p.nombre, p.apellido_pa, p.apellido_ma
                        FROM tarjeta_efecticard t
                        LEFT JOIN personal_campo p on t.id_responsable = p.id 
    ...
    public function getinfoindex($year,$month){
        try{
            ...
            $qry = $db->query("SELECT ad.id,ad.id_sitios, ad.name_sitio, ad.id_proyecto, ad.id_responsable, 
                        ad.name_responsable, ad.fecha_requerida, ad.id_vehiculo, ad.placas, ad.estacion, 
                        ad.forma_pago,ad.importe, ad.step_gasolina, ad.consumo, ad.odometro_inicial, 
                        ad.odometro_final, ad.id_tarjeta, ad.tarjeta, 
