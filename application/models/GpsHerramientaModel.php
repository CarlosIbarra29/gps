 ...
 //    }
    ...
    public function hercodcount($codigo,$offset,$no_of_records_per_page){ 
        ...
        try{
    		...
    		$qry = $db->query("SELECT h.id_herramienta, h.codigo, h.nombre, h.unidad, h.descripcion, h.imagen, h.status_disponible, 
                        h.fecha_salida, h.fecha_entrega, h.id_responsable, h.id_cat_herramienta, h.id_sitio, h.tiempo_vida, 
                        h.factura_imagen, h.fecha_compra, h.vencimiento_gar, h.marca, h.modelo, h.no_serie, h.factura_numero, p.id,
    ...
    public function Getpaginationinventario($table,$offset,$no_of_records_per_page){
        ...
        try{
    		...
    		$qry = $db->query("SELECT h.id_herramienta, h.codigo, h.nombre, h.unidad, h.descripcion, h.imagen, h.status_disponible, 
                        h.fecha_salida, h.fecha_entrega, h.id_responsable, h.id_cat_herramienta, h.id_sitio, h.tiempo_vida, 
                        h.factura_imagen, h.fecha_compra, h.vencimiento_gar, h.marca, h.modelo, h.no_serie, h.factura_numero, 
                        h.comentario_b, h.evidencia_b,p.id, IF(p.nombre is null, 'Sin Asignar', p.nombre) AS nombrea, p.apellido_pa,
                        p.apellido_ma
            			from herramienta_inventario h
...
//                 LEFT JOIN sitios s ON s.id = st.id_sitio
    ...
    public function Getdatos($wh,$id){
        ...
        try {
    		...
    		$qry = $db->query("SELECT h.id_herramienta, h.codigo, h.nombre, h.unidad, h.descripcion, h.imagen,h.status_disponible, 
                        h.fecha_salida, h.fecha_entrega, h.id_responsable, h.id_cat_herramienta, h.id_sitio, h.tiempo_vida, 
                        h.factura_imagen, h.fecha_compra,h.vencimiento_gar, h.marca, h.modelo, h.no_serie, h.factura_numero,
                        h.comentario_b, h.evidencia_b, h.ultimo_r, ch.nombre AS categoria,  
    ...
    public function Getdatos_p($table,$id){
        ...
        try {
            $db = Zend_Db_Table::getDefaultAdapter();
            $qry = $db->query("SELECT p.id, p.nombre, p.apellido_pa, p.apellido_ma, p.imagen, p.puesto, p.id_sitiopersonal, 
                        pp.id as id_puesto, pp.nombre as puestop, s.id, IF(s.nombre is NULL, 'Sin Sitio', s.nombre) AS sitio
    ...
    public function Getcobro($id){
        ...
        try {
            $db = Zend_Db_Table::getDefaultAdapter();
            $qry = $db->query("SELECT c.id_herramienta, c.id_personal, c.status_cobro, c.fecha, h.id_herramienta,h.codigo, h.nombre,
                        h.unidad, h.descripcion, h.imagen, h.status_disponible, h.fecha_salida, h.fecha_entrega, 
    ...
    public function Getherramientasres($id){
        ...
        try {
            ...
            $qry = $db->query("SELECT h.id_herramienta, h.codigo, h.nombre, h.unidad, h.descripcion, h.imagen,h.status_disponible, 
                        h.fecha_salida, h.fecha_entrega, h.id_responsable, h.id_cat_herramienta, h.id_sitio, h.tiempo_vida, 
                        h.factura_imagen, h.fecha_compra,h.vencimiento_gar, h.marca, h.modelo, h.no_serie, h.factura_numero,
                        h.comentario_b, h.evidencia_b, IF(p.nombre is null, 'Sin Asignar', p.nombre) AS nombrea, p.apellido_pa, 
                        p.apellido_ma, p.id, p.name_sitio, p.telefono, p.email_personal,  pu.nombre as pueston
                        FROM herramienta_inventario h
                        LEFT JOIN personal_campo p ON h.id_responsable = p.id 
                        LEFT JOIN puestos_personal pu ON  p.puesto = pu.id
    ...
    public function Getfecha($wh,$id){
        ...
        try {
            $db = Zend_Db_Table::getDefaultAdapter();
            $qry = $db->query("SELECT date(created_at) as fecha FROM herramienta_inventario WHERE $wh = ?",array($id));
            $row = $qry->fetchAll();
            $db->closeConnection();
            return $row;
        }
     ...
     public function GetSolicitudesReparacion($table){
        ...
        try{
            $db = Zend_Db_Table::getDefaultAdapter();
            $qry = $db->query("SELECT so.id, so.total, so.condiciones_compra, so.referencia, so.descripcion, so.name_user, 
                        so.servicio_id, so.comentario, so.name_proveedor, so.status_asignada, so.status_pago
                        FROM solicitud_ordencompra so
                        WHERE so.servicio_id = $servicio and so.status_asignada = $statusas 
                        and so.status_pago = 1 or so.servicio_id  = $servicios 
                        and so.status_asignada = $statusas and so.status_pago = 1");
            $row = $qry->fetchAll();
            return $row;
            $db->closeConnection();
        }
    ...
    public function Getnrep($id){
        ...
        try {
            $db = Zend_Db_Table::getDefaultAdapter();
            $qry = $db->query("SELECT count(id_herramienta) as Reparaciones from reportes_reparacion 
    ...
    public function reparaexcel(){  
        ...
        try{
            ...
            $qry = $db->query("SELECT r.id_reporte,r.motivo,r.orden_compra,r.fecha_inicio, r.costo,
                        ...
                        IF(r.id_solicitud = 0,'Sin solicitud Asignada',r.id_solicitud) AS asignado,
                        r.id_herramienta, h.id_herramienta as id_h, h.codigo, h.id_cat_herramienta, h.nombre, 
    ...
    public function herramientasexcel(){  
        ...
        try{
    		...
    		$qry = $db->query("SELECT h.id_herramienta, h.codigo, h.nombre, h.unidad, h.descripcion, h.imagen, 
                        IF(h.status_disponible != 2, IF(h.status_disponible != 1, 
                        IF(h.status_disponible = 0, 'Disponible', 'Baja'), 'Ocupado'),'Reparacion') AS nstatus,
                        h.status_disponible, h.fecha_salida, h.fecha_entrega, h.id_responsable,h.id_cat_herramienta, h.id_sitio, 
                        h.tiempo_vida, h.factura_imagen, h.fecha_compra, h.vencimiento_gar, h.marca, h.modelo, h.no_serie, 
                        h.factura_numero, h.comentario_b, h.evidencia_b, p.id, 
