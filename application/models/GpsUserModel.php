...
// END MODULO DE ROLES 
	...
	public function refreshPersonal($post,$table){
        try {
            $db = Zend_Db_Table::getDefaultAdapter();
            $qry = $db->query("UPDATE $table SET nombre = ?, apellido_pa = ?, apellido_ma = ?, curp = ?, puesto = ?, status_expediente=?, nss = ?, rfc = ?, telefono = ?, email_personal = ?, dia_pago = ?, hora_pago =?, disponibilidad = ?, tarjeta = ? WHERE id = ?",array(
               	$post['name'],
            	$post['apellido_pa'],
            	$post['ampellido_ma'],
            	$post['curp'],
