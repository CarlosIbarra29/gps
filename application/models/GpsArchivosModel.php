...
class Application_Model_GpsArchivosModel extends Zend_Db_Table_Abstract{
    ...
    public function getcomentarios(){
        try{
            $db = Zend_Db_Table::getDefaultAdapter();
            $qry = $db->query("SELECT c.id_sitio, s.nombre, s.id_cliente, c.comentarios  FROM comentarios_sitios c
    ...
    public function getissue($tipo){
        try{
            $db = Zend_Db_Table::getDefaultAdapter();
            $qry = $db->query("SELECT s.id,s.id_cliente, s.nombre , s.cliente, s.ciudad, s.estado, s.region, ttp.nombre_proyecto as proyecto,
                sc.nombre_status as status_cliente, sp.nombre_status as status_proyecto, st.issue
                FROM sitios s 
                INNER JOIN sitios_tipoproyecto st on s.id = st.id_sitio
                INNER JOIN status_cliente sc on sc.id = st.status_cliente
                INNER JOIN status_proyecto sp on sp.id = st.status_proyecto
    ...
    public function getvehculossolicitudes($year,$month){
        try{
            ...
            $qry = $db->query("SELECT vs.id, vs.id_responsable, vs.id_usuario, vs.id_vehiculo, vs.id_servicios, 
                ...
                vs.motivos, vs.monto, vs.status_comprobante, vs.status_asignada, vs.fecha_validacion, 
                p.nombre as nombrer, p.apellido_pa as apr, p.apellido_ma as amr, sv.nombre_servicio, pr.nombre_prov
                ,vp.fecha_pago, vp.comprobante_pago,vp.monto, vi.marca, vi.submarca, vi.modelo, vi.placas, u.nombre 
                as name_user, u.ap, u.am,
                ...
                LEFT JOIN usuario u on u.id = vs.id_usuario having mes = ? AND years = ?  order by vs.id ASC",array($month,$year));
            $row = $qry->fetchAll();
            return $row;
            $db->closeConnection();
        }catch (Exception $e){
