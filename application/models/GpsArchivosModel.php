...
class Application_Model_GpsArchivosModel extends Zend_Db_Table_Abstract{
    ...
    public function getnomiasolicitudes($year,$month){
        try{
            ...
            $qry = $db->query("SELECT pa.id, pa.id_personal, pa.nombre, pa.hora_entrada, pa.hora_salida, pa.dia, 
                        ...
                        AND pn.status_pago = 1",array($month,$year));
            $row = $qry->fetchAll();
            return $row;
            $db->closeConnection();
        }catch (Exception $e){
    ...
    public function getnomiasolicitudesproyectosajuste($month,$sitio){
        try{
            $db = Zend_Db_Table::getDefaultAdapter();
            $qry = $db->query("SELECT pa.id, pa.id_personal, pa.nombre, pa.hora_entrada, pa.hora_salida, pa.dia, 
                        pa.day_num, pa.hora_extra, pa.id_solicitudhora, pa.id_proyecto, pa.id_proyecto_salida, 
                        ...
                        having mes = ? 
                        AND pn.status_pago = 1 
                        AND pa.id_proyecto = ? AND pa.status_tipos = 0",array($month,$sitio));
            $row = $qry->fetchAll();
            return $row;
            $db->closeConnection();
        }catch (Exception $e){
    ...
    }// Cambio temporal en busca de solucion
